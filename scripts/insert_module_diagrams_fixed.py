...
def make_text_paragraph(template: ET.Element, text: str) -> ET.Element:
    ...
    template_rpr = template.find("./w:r/w:rPr", {"w": W_NS})
...
def add_relationship(rel_root: ET.Element, rel_id: str, target: str) -> None:
    ET.SubElement(
        rel_root,
...
def replace_text(root: ET.Element, replacements: list[tuple[str, str]]) -> None:
    for node in root.findall(".//w:t", {"w": W_NS}):
        if not node.text:
            continue
        text = node.text
...
def main() -> None:
    ...
    input_path = base / "（已补充模块设计）候磊-基于Unity的冒险探索RPG游戏设计与实现.docx"
    output_path = base / "（已插入模块结构图-修正版）候磊-基于Unity的冒险探索RPG游戏设计与实现.docx"
    diagram_dir = base / "paper_module_diagrams"

    temp_dir = Path(tempfile.mkdtemp(prefix="docx_insert_fixed_"))
    try:
        ...
        image_template = paragraphs[182]
        caption_template = paragraphs[183]
        blank_template = paragraphs[182]

        specs = [
            (198, "06_animation_module.svg", "图4-7 动画管理模块结构图"),
            (196, "05_scene_module.svg", "图4-6 场景管理模块结构图"),
            (194, "04_combat_module.svg", "图4-5 战斗交互模块结构图"),
            (192, "03_data_module.svg", "图4-4 数据管理模块结构图"),
            (190, "02_ui_module.svg", "图4-3 游戏UI界面模块结构图"),
            (188, "01_player_control_module.svg", "图4-2 玩家角色控制模块结构图"),
        ]

        docpr_id = next_docpr_id(root)

        for anchor_index, svg_name, caption in specs:
            current_paragraphs = [node for node in body if node.tag == qn(W_NS, "p")]
            anchor = current_paragraphs[anchor_index]
            insert_at = list(body).index(anchor) + 1

            target_name = svg_name
            shutil.copyfile(diagram_dir / svg_name, media_dir / target_name)
            rel_id = next_rel_id(rel_root)
            add_relationship(rel_root, rel_id, f"media/{target_name}")

            image_para = make_image_paragraph(image_template, rel_id, docpr_id, svg_name, 5753100, 3600000)
            docpr_id += 1
            caption_para = make_text_paragraph(caption_template, caption)
            blank_para = make_text_paragraph(blank_template, "")

            body.insert(insert_at, image_para)
            body.insert(insert_at + 1, caption_para)
            body.insert(insert_at + 2, blank_para)

        replacements = [
            ("本系统的模块结构图如下图4-2所示：", "本系统的模块结构图如下图4-1所示："),
            ("图4-2 系统模块结构图", "图4-1 系统模块结构图"),
